...
class TestInterlayerProductRegistration(unittest.TestCase):
    ...
    def test_invalid_product_data(self):
        ...
        self.assertEqual(str(cm.exception),
                         "The provided data is not rectangular.")
...
class TestInterlayerProductTable(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.interlayer = lay.Interlayer.from_product_table(self.t_pvb,
                                                            product_name)
...
class TestInvalidInterlayerClassMethods(unittest.TestCase):
    def test_invalid_product_name(self):
        ...
        self.assertEqual(str(cm.exception),
                         "The product is not registered in the product "
                         "registry.")
    ...
    def test_invalid_static_shear_mod(self):
        ...
        self.assertEqual(str(cm.exception),
                         "The shear modulus must be greater than zero "
                         "[pressure].")
...
class TestMonolithicMethod(unittest.TestCase):
    ...
    def test_h_efw(self):
        t_act = (lay.t_min_lookup_metric[self.t1nom.m_as("mm")] +
                 lay.t_min_lookup_metric[self.t2nom.m_as("mm")])*ureg.mm
...
class TestShearTransferCoefMethod(unittest.TestCase):
    ...
    def test_invalid_packages(self):
        ...
        pac_invalid_3 = [self.ply1, self.interlayer, self.ply2,
                         self.interlayer, ply3]
...
class TestGlassTypeRegistration(unittest.TestCase):
    ...
    def test_invalid_glass_type_add_1(self):
        ...
        self.assertEqual(str(cm.exception),
                         "Name identifier already in use. Deregister `Fully "
                         "Tempered` first.")
...
class TestAnnealedGlassType(unittest.TestCase):
    ...
    def test_prob_breakage_factor(self):
        self.assertAlmostEqual(self.an.prob_breakage_factor(1/1000),
                               0.681114447, 4)
        self.assertAlmostEqual(self.an.prob_breakage_factor(5/1000),
                               0.9218781646, 4)
        self.assertAlmostEqual(self.an.prob_breakage_factor(8/1000), 1, 4)
        self.assertAlmostEqual(self.an.prob_breakage_factor(10/1000),
                               1.038646695, 4)

    def test_load_duration_factor(self):
        self.assertAlmostEqual(self.an.load_duration_factor(Q_(10, "year")),
                               0.315, 3)
        self.assertAlmostEqual(self.an.load_duration_factor(Q_(12, "hour")),
                               0.550, 3)
...
class TestHeatStrengthenedGlassType(unittest.TestCase):
    ...
    def test_prob_breakage_factor(self):
        self.assertAlmostEqual(self.hs.prob_breakage_factor(1/1000),
                               0.8399834341, 4)
        self.assertAlmostEqual(self.hs.prob_breakage_factor(3/1000),
                               0.9204133016, 4)
        self.assertAlmostEqual(self.hs.prob_breakage_factor(8/1000), 1, 4)
        self.assertAlmostEqual(self.hs.prob_breakage_factor(9/1000),
                               1.010169699, 4)

    def test_load_duration_factor(self):
        self.assertAlmostEqual(self.hs.load_duration_factor(10*ureg.year),
                               0.558, 3)
        self.assertAlmostEqual(self.hs.load_duration_factor(12*ureg.hour),
                               0.739, 3)
...
class TestFullyTemperedGlassType(unittest.TestCase):
    ...
    def test_prob_breakage_factor(self):
        self.assertAlmostEqual(self.ft.prob_breakage_factor(1/1000),
                               0.9102486076, 4)
        self.assertAlmostEqual(self.ft.prob_breakage_factor(4/1000),
                               0.9679689847, 4)
        self.assertAlmostEqual(self.ft.prob_breakage_factor(8/1000), 1, 4)
        self.assertAlmostEqual(self.ft.prob_breakage_factor(10/1000),
                               1.01087724, 4)
    ...
    def test_load_duration_factor(self):
        self.assertAlmostEqual(self.ft.load_duration_factor(10*ureg.year),
                               0.678, 3)
        self.assertAlmostEqual(self.ft.load_duration_factor(12*ureg.hour),
                               0.817, 3)
...
class TestRoarks4sidePlate(unittest.TestCase):
    def setUp(self):
        self.rk4s = hp.Roarks4side(Q_(71.7, 'GPa'), Q_(5, "ft"),
                                   Q_(10, "ft"), Q_(0.5, "inch"))
    ...
    def test_stress_max(self):
        self.assertAlmostEqual(self.rk4s.stress_max(Q_(20, 'psf')),
                               Q_(8.41436178451202, "MPa"), 3)

    def test_deflection_max(self):
        self.assertAlmostEqual(self.rk4s.deflection_max(Q_(20, 'psf')),
                               Q_(-0.153704045, "in"))

    def test_reaction_max(self):
        self.assertAlmostEqual(self.rk4s.reaction_max(Q_(20, 'psf')),
                               Q_(50.3, "plf"))
...
class TestRoarks4sidePlateInvalid(unittest.TestCase):
    ...
    def test_invalid_dimension_prop_1(self):
        rk4s = hp.Roarks4side(Q_(71.7, 'GPa'), Q_(5, "ft"), Q_(10, "ft"),
                              Q_(0.5, "inch"))
        with self.assertRaises(ValueError) as cm:
            rk4s.dim_x = Q_(-10, "ft")
        self.assertEqual(str(cm.exception),
...
class TestIGUWindDemands(unittest.TestCase):
    def setUp(self):
        ...
        # Panel
        self.panel = dem.IGUWindDemands(buildup, self.wind_load, self.dim_x,
                                        self.dim_y)
        self.panel.solve()

    def test_data(self):
        self.panel.dim_x
        self.assertListEqual(self.panel.buildup,
    ...
    def test_stress_demand(self):
        self.assertAlmostEqual(self.panel.stress[self.ply1],
                               Q_(25.1955903, "MPa"), 4)
        self.assertAlmostEqual(self.panel.stress[self.ply2],
                               Q_(25.1955903, "MPa"), 4)
        self.assertAlmostEqual(self.panel.stress[self.ply3],
                               Q_(12.5977951, "MPa"), 4)

    def test_deflection_demand(self):
        self.assertAlmostEqual(self.panel.deflection[self.package1],
                               Q_(-22.9938324, "mm"), 4)
        self.assertAlmostEqual(self.panel.deflection[self.package2],
                               Q_(-22.9938324, "mm"), 4)
